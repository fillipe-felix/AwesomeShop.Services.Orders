...
namespace AwesomeShop.Services.Orders.Application.Subscribers
{
    public class PaymentAcceptedSubscriber : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (sender, evetArgs) =>
            {
                var byteArray = evetArgs.Body.ToArray();
                ...
                Console.WriteLine($"Message PaymentAccepted receveid with id {message.Id}");

                var result = await UpdateOrder(message);

                if (result)
                {
                    _channel.BasicAck(evetArgs.DeliveryTag, false);
                }
            };
